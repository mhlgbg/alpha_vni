...
import { exportServiceOrdersExcel } from "../../utils/serviceOrderExcelExport"
...
export default function SalesCounterPage() {
  ...
  async function onExportRecentOrdersExcel() {
    if (recentOrdersExporting) return
    if (!activeDepartment?.id || !employee?.id || !canViewRecentOrders) return
    ...
    try {
      const pageSize = 200
      let page = 1
      let pageCountFromApi = 1
      const allRows = []
      const today = toLocalDateYmd()

      while (page <= pageCountFromApi) {
        const params = {
          page,
          pageSize,
          sort: "orderDate:desc",
        ...
        const payload = await getServiceOrders(params)
        const rowsInPage = normalizeApiRows(payload)
        const meta = payload?.meta?.pagination || {}

        allRows.push(...rowsInPage)

        pageCountFromApi = Number(meta.pageCount || 1)
        page += 1
      }

      await exportServiceOrdersExcel({
        orders: allRows,
        filePrefix: "sales_counter_orders",
      })
    } catch (exportError) {
                        ...
                        <CCardHeader className="d-flex justify-content-between align-items-center gap-2 flex-wrap">
                          ...
                          <div className="d-flex align-items-center gap-2">
                            ...
                            <CButton
                              ...
                              color="success"
                              variant="outline"
                              disabled={recentOrdersLoading || recentOrdersExporting || !canViewRecentOrders}
                              onClick={onExportRecentOrdersExcel}
                            >
